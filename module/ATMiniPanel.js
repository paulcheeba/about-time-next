...
#${PANEL_ID} .atmp-time { font-size: 12px; font-weight: 700; text-align: center; white-space: nowrap; }
#${PANEL_ID} .atmp-buttons, #${PANEL_ID} .atmp-buttons-td {
  display: grid; grid-auto-flow: column; grid-auto-columns: max-content; gap: 6px; justify-content: center; align-items: stretch; margin-top: 8px; }
...
#${PANEL_ID} .atmp-pill { position: absolute; top: -10px; left: 10px; font-size: 10px; padding: 2px 6px; border-radius: 6px;
  background: rgba(255, 180, 0, .2); border: 1px solid rgba(255, 180, 0, .35); color: #ffda7b; }
...
function buildPanel() {
  ...
  inner.append(pill, title, time, ctlRow, steps, tod);
  root.append(grip, closeBtn, inner);
  document.body.appendChild(root);
  return { root, steps, tod, pill, timeEl: time, playPauseBtn: playPause };
}
...
function wireBehavior(ctx) {
  ...
  if (game.user.isGM) {
    ...
    steps.querySelectorAll(".atmp-btn").forEach((b) => b.addEventListener("click", onStep));
  }
