...
// v13.0.6.2 — Windowless, draggable mini time panel (GM=controls; players=time-only)
...
/** Parse /at-style duration tokens (e.g., 1h5m3s, 90s, 2m). Returns seconds (integer >=0). */
function parseDuration(input) {
  if (!input || typeof input !== "string") return 0;
  const s = input.trim().toLowerCase();
  if (/^\d+$/.test(s)) return parseInt(s, 10); // plain seconds
  let total = 0;
  const re = /(\d+)\s*([dhms])/g;
...
function loadPos() {
  try { return JSON.parse(localStorage.getItem(POS_KEY) || "null") ?? { top: 80, left: 120 }; }
  catch { return { top: 80, left: 120 }; }
}
...
function ensureStyles() {
  ...
  style.textContent = `
#${PANEL_ID} {
  position: fixed; z-index: 99999; display: inline-block;
...
#${PANEL_ID} .atmp-inner {
  ...
  border-radius: 8px; background: var(--color-bg-alt, rgba(255,255,255,0.05));
...
#${PANEL_ID} .atmp-title { font-size: 8px; font-weight: 600; opacity: 0.9; margin: 0 0 4px 0; text-align: center; }
...
#${PANEL_ID} .atmp-btn { display: inline-flex; flex-direction: column; align-items: center; justify-content: center; gap: 2px; padding: 4px 8px; border-radius: 6px; background: var(--color-background, rgba(255,255,255,0.05)); border: 1px solid var(--color-border-light-1, rgba(255,255,255,0.18)); cursor: pointer; white-space: nowrap; }
...
function buildPanel() {
  ...
  buttons.append(
    mk("rwd1", durs.rwd1, "fa-angles-left",  `Rewind ${durs.rwd1}`),
    mk("rwd2", durs.rwd2, "fa-angles-left",  `Rewind ${durs.rwd2}`),
    mk("fwd1", durs.fwd1, "fa-angles-right", `Advance ${durs.fwd1}`),
    mk("fwd2", durs.fwd2, "fa-angles-right", `Advance ${durs.fwd2}`),
    mk("fwd3", durs.fwd3, "fa-angles-right", `Advance ${durs.fwd3}`),
  );
...
function makeDraggable(el) {
  ...
  function down(ev) {
    ...
    const r = el.getBoundingClientRect(); baseL = r.left; baseT = r.top;
    ev.preventDefault();
  }
  function move(ev) {
...
export function hideMiniPanel() {
  if (!_visible) return;
  _closer?.(); _closer = null; _visible = false;
}
